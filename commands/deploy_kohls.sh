...
echo 'RUN: sed -i "s/goldrush_prod_web:1.3.0/goldrush_cookie_kohls:latest/" Dockerrun.aws.json ...'
sed -i "s/goldrush_prod_web:1.3.0/goldrush_cookie_kohls:latest/" Dockerrun.aws.json

echo 'RUN: git add Dockerrun.aws.json ...'
git add Dockerrun.aws.json
echo "RUN: git commit -m 'kohls ${versionname}' ..."
